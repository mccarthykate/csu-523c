...
> I was able to remove a little over 11,000 points from the boundary through simplification. This means there will be fewer computaions, which will make any calculations run faster. However, this may not capture fine details along the coastline, so it's important to simplify based on the computational objective. 
...
max(log10(flood_count$n.y))
max(log10(fire_count$n.y))
max(log10(recreation_count$n.y))
max(log10(water_supply_count$n.y))

fire_dams_plot <- pip_plot(fire_count, "Distribution of Dams for Fire Protection by County")

recreation_dams_plot <-

water_supply_dams_plot <- pip_plot(water_supply_count, "Distribution of Dams for Water Supply by County")

flood_dams_plot <- pip_plot(flood_count, "Distribution of Dams for Flood Risk Reduction by County")

fire_dams_plot
recreation_dams_plot
water_supply_dams_plot
flood_dams_plot
