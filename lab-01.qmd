...
low_flow_threshold <- low_flow_stats %>% 
  ...
  summarize(threshold = quantile(rolling_mean, probs = 0.1, na.rm = T))
```
...
```{r}
low_flow_stats_threshold <- low_flow_stats %>% 
  left_join(low_flow_threshold, by = "name")
  
mutate(low_flow_stats_threshold, below_threshold = rolling_mean < threshold)
```
...
lowflow_leesferry_plot <- lowflow_leesferry %>% 
  ...
  geom_line() + 
  geom_hline(yintercept = 8824, linetype = "dashed") + 
  geom_ribbon(aes(ymin = 0, ymax = 8824), fill = "red", alpha = 0.5) + 
  labs(title = "Lees Ferry Flow from 2000-2023", 
       ...
       y = "Rolling Mean (cfs)", 
       caption = "Data from USGS gauges. Threshold limit set by the 10th percentile of 7-day rolling mean from 1980-2010.") + 
  theme_bw()

print(lowflow_leesferry_plot)
```
...
```{r}
count_lowflow_2000_2010 <- lowflow_leesferry %>% 
  filter(Date <= as.Date("2010-12-31")) %>% 
  summarize(low_days = sum(Flow < threshold),
            year_number = 11, 
            avg_lfdays_per_year = low_days/year_number) %>% 
  mutate(Period = "2000-2010")

count_lowflow_2011_2023 <- lowflow_leesferry %>% 
  filter(Date > as.Date("2010-12-31")) %>% 
    summarize(low_days = sum(Flow < threshold),
            year_number = 13, 
            avg_lfdays_per_year = low_days/year_number) %>% 
...
full_join_leesferry <- full_join(count_lowflow_2000_2010, count_lowflow_2011_2023)

leesferry_summarytable <- flextable(full_join_leesferry) %>% 
  set_header_labels(low_days = "Total Low-Flow Days", year_number = "Length of Time Period", Period = "Time Period", avg_lfdays_per_year = "Average Number of Low Flow Days per Year") %>% 
  set_caption(caption = "Lees Ferry Low Flow-Days Averaged per Year from 2000-2023")
print(leesferry_summarytable)
```

> The below-threshold days (on a yearly average) decreased from 2011-2023 compared to 2000-2010. It looks to be a shift in isolated dry years rather than a baseline change, because the low flow graph demonstrates a pretty consistent number of low flow years in both time periods. 

*FINISH ANSWERING THIS QUESTION LATER AND CITE A SOURCE* 
...
water_year_leesferry <- water_year_data %>% 
  filter(name == "Colorado R at Lees Ferry", water_year != "2024")%>% 
  mutate(median_discharge = median(annual_discharge))

wy_leesferry_plot <- water_year_leesferry %>% 
  ggplot(mapping = aes(x = water_year, y = annual_discharge, fill = annual_discharge < median_discharge)) + 
  geom_col() + 
  scale_fill_manual(
    values = c("TRUE" = "darkred", "FALSE" = "dodgerblue4"),
    labels = c("TRUE" = "Below Median", "FALSE" = "Above Median"),
    name = "Flow Status") +
  geom_hline(aes(yintercept = median_discharge), linetype = "dashed") + 
  labs(x = "Water Year", 
       y = "Annual Discharge (cfs)", 
       title = "Annual Discharge at Lees Ferry Gauge on the Colorado River", 
       caption = str_wrap("Dashed line represents the median annual discharge, with blue bars being water years above the median, and red bars below. Data from USGS gauges.", width = 100)) 
