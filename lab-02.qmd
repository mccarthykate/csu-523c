...
#leaflet map 
leaflet() %>%
  addProviderTiles(providers$CartoDB.Positron, group = "Light Map") %>% #base layer
  addProviderTiles(providers$Esri.WorldImagery, group = "Satellite") %>% #base layer
  addPolygons(data = states_4326, fillColor = "grey90", color = "black", weight = 1, group = "States") %>% #state layer
  addCircleMarkers(data = cities_4326, group = "All Cities", radius = 3, weight = 1, #cities 
                   color = "white", fillOpacity = 0.7, fillColor = ~palette(dist_to_border),
                   popup = mapply(make_label, split(cities_4326, seq(nrow(cities_4326))), SIMPLIFY = FALSE),
                   clusterOptions = markerClusterOptions(disableClusteringAtZoom = 5)) %>% 
  addCircleMarkers(data = equidistant_mex_can_4326, group = "Equidistant Zone", #equidistant zone
                   radius = 3, weight = 1, fillOpacity = 0.7, color = "seagreen",  
                   popup = mapply(make_label, split(equidistant_mex_can_4326, seq(nrow(equidistant_mex_can_4326))), SIMPLIFY = FALSE)) %>% 
  addCircleMarkers(data = border_cities_4326, #border zone 
                   group = "Border Zone", 
                   radius = 3, weight = 1, fillOpacity = 0.7, color = "steelblue",
                   popup = mapply(make_label, split(border_cities_4326, seq(nrow(border_cities_4326))), SIMPLIFY = FALSE)) %>%
  addLayersControl(
    baseGroups = c("Light Map", "Satellite"),
    overlayGroups = c("States", "All Cities", "Equidistant Zone", "Border Zone"),
    options = layersControlOptions(collapsed = FALSE)) %>% #make layers 
  addLegend(pal = palette, values = cities$dist_to_border, title = "Distance to National Border (km)", 
            position = "bottomright") %>% #cities legend (dist to border)
  addLegend(colors = c("seagreen", "steelblue"), labels = c("Equidistant Zone", "Border Zone"), 
            opacity = 0.7, title = "Zones", position = "topleft") #zones legend
...
> st_transform() was used to convert the "eqdc" coordinate refernce system to leaflet-compatable coordinates, setting the CRS to WGS 2346, which is the standard CRS. For the cities layer depicting the distance to the national border of all cities in the contiguous United States, the point are clustered until zoom level 5, which allows the map to look cleaner on the national scale. The equidistant and border zone points get over-shadowed when the cities layer isn't clustered, so they needed to remain grouped until zoomed to a smaller region. 
