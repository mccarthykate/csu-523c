...
leaflet() %>%
  ...
  # state boundaries
  addPolygons(data = states_4326, fillColor = "grey90", color = "black", weight = 1, group = "States") %>%
  # all city markers - FIX: use mapply to apply function row-wise
  ...
  addCircleMarkers(data = border_cities_4326, 
                   group = "Border Zone", 
                   radius = 3, weight = 1, fillOpacity = 0.7, color = "dodgerblue4",
                   popup = mapply(make_label, split(border_cities_4326, seq(nrow(border_cities_4326))), SIMPLIFY = FALSE)
  ) %>%
  # layer controls - FIX: use correct group names
  addLayersControl(
  ...
  addLegend(colors = c("seagreen", "dodgerblue4"), labels = c("Equidistant Zone", "Border Zone"), 
            opacity = 0.7, title = "Zones", position = "topleft")
  #addLegend(pal = palette, values = cities$dist_to_border, title = "Distance (km)", 
