...
> answer: zero_q_freq represents the frequency of days with Q = 0 mm/day, which is represented as a percentage. 
...
p_mean_plot <- ggplot(data = camels, aes(x = gauge_lon, y = gauge_lat)) +
  ...
  theme_map() + 
  labs(title = "Average Preciptation of Basins in CONUS", 
       caption = "Data from N15 - Daymet*.", 
       color = "Average Precipitaiton (mm/day)") + 
...
aridity_plot <- ggplot(data = camels, aes(x = gauge_lon, y = gauge_lat)) +
  ...
  theme_map() + 
  labs(title = "Aridity of Basins in CONUS", 
       caption = "Aridity calculated as the ratio of mean PET to mean precipitation Data from N15 - Daymet*.", 
       color = "Aridity") + 
  theme(legend.position = "bottom")

aridity_plot + p_mean_plot +
...
> answer: The workflow and the base implementation gave identical model coefficients, which means we're doing good.  
...
#add new models to workflow set 
...
> answer: Even though the neural network ranked first between each of the models, I would move forward with the random forest model. Neural networks aren't always reliable because they can be a "black box" so it's hard to see *how* the model got to the prediction. To avoid this issue, moving forward with the second-best model would be the smart choice. 
...
#visualize distribution of q_mean and chosen predictors 
q_mean_dist <- ggplot(camels, aes(x = q_mean)) +
  geom_histogram(bins = 25)

q_mean_dist

camels_vis <- camels %>% 
  select(p_mean, pet_mean, frac_snow, aridity, elev_mean, slope_mean, frac_forest, soil_porosity, soil_conductivity, max_water_content, sand_frac, silt_frac, clay_frac) %>% 
...
model_camels <- camels %>% 
  select(gauge_lon, gauge_lat,p_mean, pet_mean, frac_snow, aridity, elev_mean, slope_mean, frac_forest, soil_porosity, soil_conductivity, max_water_content, sand_frac, silt_frac, clay_frac) %>% 
  drop_na() 
  
dists <- as.matrix(dist(model_camels %>% select(gauge_lon, gauge_lat)))
...
model_camels %>% 
  select(-gauge_lon, -gauge_lat) %>% #drop lat and long because they will obvs have high spatial autocorrelation
...
(resample_split <- initial_split(camels, prop = 0.75, strata = logQmean))

#sanity check --> make sure 75/25 split actually occured 
168/671

#extract training and testing sets 
model_camels_train <- training(resample_split)
model_camels_test <- testing(resample_split)
```

> answer:  The data contains 53 numeric variables and 6 character variables. Some of the numeric data has missing values, particularly root_depth_50 and root_depth_99, which both have 24 missing values. However, because of the size of the dataset, these can be interpolated from the mean to fill in the N/As for these variables. Many of the numeric variables, particularly the ones related to discharge (Q) have right-skewed distributions, which can likely be log-transformed to increase linearity. When looking at the distribution of q_mean, the histogram demonstrates this right-skewed quality, so the output variable should be the log of q_mean. Aridity, mean elevation, fraction snow, mean precipitation, mean slope, and soil conductivity are all right-skewed, so they will need to be log-transformed. Stratification by logQmean was chosen so that the output variable is accurately represented. 
...
```{r}
recipe_obj <- recipe(logQmean ~ ., data = camels_train) %>%
  step_rm(q_mean, runoff_ratio, slope_fdc, baseflow_index, stream_elas,q5, q95, high_q_freq, high_q_dur, low_q_freq, low_q_dur, zero_q_freq, hfd_mean, gauge_lat, gauge_lon, gauge_id, high_prec_timing, low_prec_timing, root_depth_50, root_depth_99, geol_porostiy, geol_1st_class, geol_2nd_class, dom_land_cover) %>% 
  step_impute_mean() %>% 
...
> answer: I chose to predict on aridiy, mean elevation, snow fraction, mean precipitation, mean slope, and soil conductivity. These were chosen because they all impact discharge, but don't have as much spatial correlation as featuers like longitude and latitude. I also chose to leave out any predictors directly related to discharge, such as mean discharge and runoff ratio. I attempted to minimize the number parameters while balancing the efficacy of the model. The exploratory data analysis revealed that aridity, mean elevation, snow fraction, mean precipitation, mean slope, and soil conductivity are skewed right, so a step_log function was used to redistribute the data. All predictors were transformed to a common scale using step_normalize, which ensures that all predictors contribute to the model equally, without magnitude impacting it. Laslty, na_omit was used to remove and N/As from predictors and outcomes. 
...
```{r}
#random forest
rf_model_mine <- rand_forest() %>%
  set_engine("ranger", importance = "impurity") %>%
  set_mode("regression")

#xgboost
xgb_model_mine <- boost_tree() %>% 
  set_engine("xgboost") %>% 
  set_mode("regression")

#decision tree 
dt_model_mine <- decision_tree() %>% 
  set_engine("rpart") %>% 
  set_mode("regression")
```

> answer: Random forest models construct multiple decision trees and aggregates predictions to improve accuracy and reduce overfitting. A boosted tree model use powerful machine learning method to improve predictive accuracy and reduce bias. Decision models have a flowchart-like structure and can capture non-linear relationships.   
...
camels_block <- spatial_block_cv(camels_sf, v = 10)

wf_set <- workflow_set(preproc = list(recipe_obj),
                       models = list(rf_model_mine, 
                                     xgb_model_mine, 
                                     dt_model_mine))
...
rank_results(wf_random, rank_metric = "rmse", select_best = TRUE)

rank_results(wf_spatial, rank_metric = "rmse", select_best = TRUE)

autoplot(wf_random) / autoplot(wf_spatial)
```

> answer: With the rank metric being root mean squared error, the random forest model ranked first in the random CV and the boosted tree model came in first for the spatial CV. However, the random forest model ranks first when comparing R^2 values in both the random and spatial CV. When going from a random to a spatial CV, performance dropped about 0.07 rmse when comparing across models. This is a relatiely small gap, which indicates some spatial correlation. Because this gap between cross validations is so small, it signals that the predictors carry real transferable signal. 
...
```{r}
#chose random forest model because it ranked higher on most metrics (rmse for spatial and rsq overall)
...
#build grid
rf_tuned_grid <- grid_space_filling(
  mtry(range = c(2, 11)),
  min_n(range = c(2, 20)),
  size = 20)

#tune against spatial CV
rf_tuned_spatial <- tune_grid(rf_tuned,
  preprocessor = recipe_obj,
  resamples = camels_block,
  grid = rf_tuned_grid,
  control = control_grid(save_pred = TRUE))

autoplot(rf_tuned_spatial)

show_best(rf_tuned_spatial, metric = "rsq")
select_best(rf_tuned_spatial, metric = "rsq")
```
 
> answer: The best hyperparameter combination was mtry = 11 and min_n = 5, which resulted in a rsq of .876. It looks like the model is sensitive to the mtry parameter compared to min_n. 
...
best_params <- select_best(rf_tuned_spatial, metric = "rsq")

#use best parameters to finalize wf 
final_wf <- finalize_workflow(rf_tuned_wf, best_params)
...
> answer: The top predictors in terms of importance were mean precipitation, aridity, low precipitation freqency. This makes sense because precipitation and aridity affect the amount of water in a stream. However, these predictors are spatially correlated in the US, with wetter regions in the east and drier regions in the west. Soil predictors rank relatively low (soil depth, percent clay), with percent sand and percent silt not even making the top 20 in terms of variable importance. 
...
final_last_fit <- last_fit(final_wf, camels_split)
...
collect_predictions(final_last_fit) %>%
  bind_cols(testing(camels_split) %>%
              select(p_mean)) %>%
  ggplot(aes(x = logQmean, y = .pred, color = p_mean)) +
  scale_color_viridis_c() +
  geom_point() +
  geom_abline(linetype = "dashed") +
  geom_smooth(method = "lm", color = "red3", se = FALSE) +
  theme_linedraw() +
  labs(title = "Log Mean Streamflow: Observed vs Predicted",
       x = "Observed Log Mean Flow",
       y = "Predicted Log Mean Flow",
       color = "Mean Precipitation")
```

> answer: The final random forest tuned model has an root mean squared error of 0.36 and an R^2 of 0.902. The model underpredicts at higher observd log mean streamflow. I am curious to see how the model would change if fewer predictors were put into the model, rather than throwing the whole kitchen sink at it (minus predictors directly related to streamflow). 
