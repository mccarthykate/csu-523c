...
> The dimensions of the stacked image are 7801 rows by 7681 columns by 6 layers. The CRS is WGS 84 / UTM zone 15N (EPSG:32615). The cell resolution of the stacked image is 30m by 30m.
...
```{r}
#transform AOI to landsat stack CRS 
palo_utm <- bbox_palo %>% 
  st_transform(crs(landsat_stack))

#crop raster stack to AOI 
landsat_cropped <- crop(landsat_stack, palo_utm)

#check cropped image 
cat("\nCropped extent:\n")

print(ext(landsat_cropped))

cat("Cropped dimensions:", dim(landsat_cropped), "\n")
```
...
```{r}
#RGB (natural color)
plotRGB(landsat_cropped, r = 4, g = 3, b = 2, stretch = "hist",
        main = "Natural Color: Red-Green-Blue")

#NIR-R-G (color infrared)
plotRGB(landsat_cropped, r = 5, g = 4, b = 3, stretch = "lin",
        main = "Color Infrared: NIR-R-G")

#NIR-SWIR1-R (false color water focus)
plotRGB(landsat_cropped, r = 5, g = 6, b = 4, stretch = "lin",
        main = "False Color (Water Focus): NIR-SWIR1-R")

#SWIR1-NIR-R (vegetation analysis) -- my choice
plotRGB(landsat_cropped, r = 6, g = 5, b =4, stretch = "lin",
        main = "Vegetation Analysis: NIR-SWIR1-B ")

```

> The natural color image shows water as dark, vegetation as green, urban areas as gray/tan, and clouds as white. However, flooding can look similar to vegetation in this representation. This would be a beneficial representation for analyzing the state of water bodies and sedimentation processes. The color infrared image shows the healthy vegetation as bright red or magenta, water as dark blue/black, soil/urban areas as tan/gray. This is good for determining vegetation compared to water. The water-focused false color. image shows open water or flodded areas as dark blue, and vegetation is bright colors (such as light green/orange/yellow). This representation clearly delineates water boundaries. Laslty, I did a vegetation analysis image, which shows water as dark blue, vegetation as bright green, and bare soil as pink. This would be a good representation when studying vegetation or monitoring crops because you can see a clear difference between bare soil and healthy vegetation.
...
```{r}
#ndvi formula and plot
ndvi <- (landsat_cropped$nir08 - landsat_cropped$red) / (landsat_cropped$nir08 + landsat_cropped$red) 
plot(ndvi, col = colorRampPalette(c("blue", "white", "red"))(256))

#ndwi formula and plot
ndwi <- (landsat_cropped$green - landsat_cropped$nir08) / (landsat_cropped$green + landsat_cropped$nir08)
plot(ndwi, col = colorRampPalette(c("blue", "white", "red"))(256))

#mndwi formula and plot
mndwi <- (landsat_cropped$green - landsat_cropped$swir16) / (landsat_cropped$green + landsat_cropped$swir16)
plot(mndwi, col = colorRampPalette(c("blue", "white", "red"))(256))

#wri formula and plot
wri <- (landsat_cropped$green + landsat_cropped$red) / (landsat_cropped$nir08 + landsat_cropped$swir16)
plot(wri, col = colorRampPalette(c("blue", "white", "red"))(256))

#swi formula and plot
swi <- 1 / (sqrt(landsat_cropped$blue - landsat_cropped$swir16))
plot(swi, col = colorRampPalette(c("blue", "white", "red"))(256))

```

> Describe the 5 images. How are they similar and where do they deviate?

> answer: The swi and ndvi plots both depict water as blue, whereas the ndwi, mndwi, and wri plots all depict water as red. The mndwi and wri plots both depict water as red, and bare ground as blue, which gives a clear distinction of water delineations. The swi plot is the only one of the five that masks values other than what is considered water.
...
ndvi_flood  <- app(ndvi,  function(x) ifelse(x < 0, 1, 0)) %>% 
  setNames("Flood_Status") # NDVI < 0 = water

ndwi_flood <- app(ndwi, function(x) ifelse(x > 0, 1, 0)) %>% 
  setNames("Flood_Status") # NDWI > 0 = water 

mndwi_flood <- app(mndwi, function(x) ifelse(x > 0, 1, 0)) %>% 
  setNames("Flood_Status") # MNDWI > 0 = water 

wri_flood <- app(wri, function(x) ifelse(x > 1, 1, 0)) %>% 
  setNames("Flood_Status") # WRI > 1 = water 

swi_flood <- app(swi, function(x) ifelse(x < 5, 1, 0)) %>% 
  setNames("Flood_Status") # SWI < 5 = water 

index_flood_stack <- c(ndvi_flood, ndwi_flood, mndwi_flood, wri_flood, swi_flood) %>% setNames(c("NDVI Flood", "NDWI Flood", "MNDWI Flood", "WRI Flood", "SWI Flood"))

index_flood_stack <- app(index_flood_stack, function(x) ifelse(is.na(x), 0, x))

#plot: blue = flooded, white = non-flooded
plot(index_flood_stack, col = c("white", "blue"),
    levels = c("non-flooded", "flooded"),
    type = "classes")
```
...
> Each of the maps are very similar and classify flooded vs non-flooded almost identically. The modified normalized difference water index shows the largest extent classified as "flooded." On the other hand, the simple water index classifies the smallest portion of the AOI as flooded. The range in classification and representation in the maps highlights the difference in indices. It's vital to understand which index would best classify the variable observed when conducting a remote-sensing analysis.
...
values = c(landsat_cropped$coastal, landsat_cropped$blue, landsat_cropped$green, landsat_cropped$red, landsat_cropped$nir08, landsat_cropped$swir16) %>% 
  values()

dim(values)
```

> The dimensions of the extracted values is 12192, 6. This tells me that the function extracted the reflectance data from each of the spectral bands (coastal, blue, green, red, NIR, SWIR1). 
...
```{r}
#cluster 
#kmeans12 <- kmeans(values, 12, iter.max = 100)
#kmeans10 <- kmeans(values, 10, iter.max = 100)
kmeans5 <- kmeans(values, 5, iter.max = 100) #best choice
```
...
```{r}
kmeans_raster = landsat_cropped[[1]]
...
kmeans_raster[idx] <- kmeans5$cluster

plot(kmeans_raster, col = RColorBrewer::brewer.pal(5, "Paired"), main = "Palo, Iowa Flood Extent (k = 5)") #best choice 

#clus_raster[idx] <- kmeans12$cluster

#plot(clus_raster, col = RColorBrewer::brewer.pal(12, "Paired"), main = "Palo, Iowa Flood Extent (k = 12)") 

#clus_raster[idx] <- kmeans10$cluster

#plot(clus_raster, col = RColorBrewer::brewer.pal(10, "Paired"), main = "Palo, Iowa Flood Extent (k = 10)") 

```

Try a few different clusters (k) to see how the map changes.

> I chose 5 clusters because the AOI doesn't have too much variation. Both 12 and 10 clusters made too many clusters for how little the features vary in the remote sensing image. By having too many clusters, the similar variables would ultimately be classed differently in order to meet the cluster value.  
...
flood_stack <- c(index_flood_stack, kmeans_flood) %>%
  setNames(c("NDVI", "NDWI", "MNDWI", "WRI", "SWI", "k_means"))

flood_counts <- global(flood_stack, "sum", na.rm = TRUE)

flood_counts$area_km2 <- flood_counts$sum * 900 / 1000000
...
```{r}
flood_uncert <- app(flood_stack, sum, na.rm = TRUE)

plot(flood_uncert, col = blues9, main = "Flood Certainty", sub = "6 = certain, 1 = uncertain")
```
...
```{r}
summed_raster <- flood_uncert

values(summed_raster)[values(summed_raster) == 0] <- NA

mapview(summed_raster, col.regions = blues9, layer.name = "Flood Certainty")
```

> The cell values depict how many methods identified that cell as "flooded" or "not-flooded." In this case, they are all whole numbers becuase the plot is adding how many methods determined that cell to be flooded. Addition of all whole numbers results in whole numbers. Therfore, the cell values range from 1-6, depicting the certainty of that cell being classified as flooded. 
